# Before running the script in the server remember to use:
...
save_render = False # TODO: change this
        ...
        for fidx in trange(total_frames):
          ...
          for batch in dataloader:
            ...
            for n in range(batch_size):
              ...
              mano_data = {
                'focal' : [np.float64(scaled_focal_length), np.float64(scaled_focal_length)],
                'princpt': [np.float64(img_size[n][0] / 2), np.float64(img_size[n][1] / 2)],
                'kpt2d' : kpts_2d,
                'transl' : cam_t.astype(np.float64),
                'is_right' : is_right,
              }
              curr_out_dir[n] = mano_data
          ...
          out_npy_result.append(curr_out_dir)

          # Render front view
          if len(all_verts) > 0 and save_render and fidx<n_frames_to_render:
            misc_args = dict(
                mesh_base_color=LIGHT_PURPLE,
                scene_bg_color=(1, 1, 1),
                focal_length=scaled_focal_length,
                )
            cam_view = renderer.render_rgba_multiple_osmesa(all_verts, cam_t=all_cam_t, render_res=img_size[n], is_right=all_right, **misc_args)
